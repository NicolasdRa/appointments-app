...
export const BookAppointmentForm = () => {
	...
	const currentDoctor = useAppSelector<Doctor | undefined>(
		selectCurrentDoctor,
	);
	...
	const [values, setValues] = useState<Appointment>({
		...
		time: '',
		user: '',
		doctor: currentDoctor ? `Dr. ${currentDoctor.name.last}` : '',
		description: '',
	});
			...
			<form onSubmit={handleSubmit} encType='multipart/form-data'>
				...
				<DialogContent>
					<Typography variant='body2' className={classes.subtitle}>
						Complete the requestedd info and book your appointment.
					...
					<FormControl className={classes.formControl}>
						...
						<Select
							...
							value={
								currentDoctor
									? currentDoctor.name.last
									: values.doctor
							}
							onChange={handleInputChange}>
							{doctors.map((doctor, index) => (
								<MenuItem key={index} value={doctor.name.last}>
									{`Dr. ${doctor.name.last}`}
								</MenuItem>
							))}
						</Select>
